import React, { useEffect, useState } from "react";

const Door = ({
  index,
  isPrizeThere,
  firstChoice,
  setFirstChoice,
  openedByHost,
  setGameWon,
  setScoreCard,
  setCurtain,
}) => {
  const [openDoor, setOpenDoor] = useState(false);

  const handleDoorClick = () => {
    if (!firstChoice?.opened) {
      setFirstChoice({ opened: true, doorIndex: index });
      return;
    }

    setOpenDoor(true);
    setCurtain(true);
    if (isPrizeThere) {
      setScoreCard((curr) => ({ ...curr, wins: curr?.wins + 1 }));
      setGameWon(true);
    } else setScoreCard((curr) => ({ ...curr, losses: curr?.losses + 1 }));
  };

  useEffect(() => {
    if (openedByHost === index) setOpenDoor(true);
  }, [openedByHost]);

  return (
    <div
      className={`h-[400px] cursor-pointer shadow-lg w-[250px] flex items-center justify-center ${
        openDoor ? "" : "bg-gray-500"
      } border-[3px] border-brown-400`}
      onClick={handleDoorClick}
    >
      {!openDoor && (
        <p className="font-bold text-white text-3xl">Door {index + 1}</p>
      )}
      {openDoor &&
        (isPrizeThere ? (
          <svg
            xmlns="http://www.w3.org/2000/svg"
            // xmlns:xlink="http://www.w3.org/1999/xlink"
            version="1.0"
            id="Layer_1"
            width="100px"
            height="100px"
            viewBox="0 0 64 64"
            enable-background="new 0 0 64 64"
            // xml:space="preserve"
          >
            <g>
              <g>
                <path
                  fill="#45AAB8"
                  d="M27,8h10c17.495,0,18,21.544,18,24h2c0-2.661-0.533-26-19-26H26C18.904,6,7,9.378,7,32h2    C9,16.523,15.393,8,27,8z"
                />
                <path
                  fill="#45AAB8"
                  d="M58,34H6c-2.206,0-4,1.794-4,4v10c0,2.206,1.794,4,4,4h52c2.206,0,4-1.794,4-4V38    C62,35.794,60.206,34,58,34z"
                />
              </g>
              <g>
                <path
                  fill="#F9EBB2"
                  d="M50.581,19.394C47.909,13.16,43.34,10,37,10H27c-7.299,0-16,3.816-16,22h5c0-4.418,3.582-8,8-8    s8,3.582,8,8h21C53,30.367,52.825,24.632,50.581,19.394z"
                />
                <path
                  fill="#F9EBB2"
                  d="M24,26c-3.313,0-6,2.687-6,6h12C30,28.687,27.313,26,24,26z"
                />
              </g>
              <g>
                <path
                  fill="#394240"
                  d="M58.982,32.088C58.985,32.057,59,32.031,59,32c0-2.866-0.589-28-21-28H26C12.654,4,5,14.206,5,32    c0,0.031,0.015,0.057,0.018,0.088C2.176,32.559,0,35.027,0,38v10c0,3.309,2.691,6,6,6v5c0,0.553,0.447,1,1,1h8    c0.553,0,1-0.447,1-1v-5h32v5c0,0.553,0.447,1,1,1h8c0.553,0,1-0.447,1-1v-5c3.309,0,6-2.691,6-6V38    C64,35.027,61.824,32.559,58.982,32.088z M26,6h12c18.467,0,19,23.339,19,26h-2c0-2.456-0.505-24-18-24H27C15.393,8,9,16.523,9,32    H7C7,9.378,18.904,6,26,6z M24,24c-4.418,0-8,3.582-8,8h-5c0-18.184,8.701-22,16-22h10c6.34,0,10.909,3.16,13.581,9.394    C52.825,24.632,53,30.367,53,32H32C32,27.582,28.418,24,24,24z M30,32H18c0-3.313,2.687-6,6-6S30,28.687,30,32z M14,58H8v-4h6V58z     M56,58h-6v-4h6V58z M62,48c0,2.206-1.794,4-4,4H6c-2.206,0-4-1.794-4-4V38c0-2.206,1.794-4,4-4h52c2.206,0,4,1.794,4,4V48z"
                />
                <path
                  fill="#394240"
                  d="M11,39c-2.206,0-4,1.794-4,4s1.794,4,4,4s4-1.794,4-4S13.206,39,11,39z M11,45c-1.103,0-2-0.897-2-2    s0.897-2,2-2s2,0.897,2,2S12.103,45,11,45z"
                />
                <path
                  fill="#394240"
                  d="M53,39c-2.206,0-4,1.794-4,4s1.794,4,4,4s4-1.794,4-4S55.206,39,53,39z M53,45c-1.103,0-2-0.897-2-2    s0.897-2,2-2s2,0.897,2,2S54.103,45,53,45z"
                />
                <path
                  fill="#394240"
                  d="M43,40H21c-0.553,0-1,0.447-1,1s0.447,1,1,1h22c0.553,0,1-0.447,1-1S43.553,40,43,40z"
                />
                <path
                  fill="#394240"
                  d="M43,44H21c-0.553,0-1,0.447-1,1s0.447,1,1,1h22c0.553,0,1-0.447,1-1S43.553,44,43,44z"
                />
                <path
                  fill="#394240"
                  d="M43.293,18.708c0.195,0.195,0.451,0.293,0.707,0.293s0.512-0.098,0.707-0.293    c0.391-0.391,0.391-1.023,0-1.414l-2-2c-0.391-0.391-1.023-0.391-1.414,0s-0.391,1.023,0,1.414L43.293,18.708z"
                />
                <path
                  fill="#394240"
                  d="M43.293,23.707C43.488,23.902,43.744,24,44,24s0.512-0.098,0.707-0.293c0.391-0.391,0.391-1.023,0-1.414    l-7-7c-0.391-0.391-1.023-0.391-1.414,0s-0.391,1.023,0,1.414L43.293,23.707z"
                />
              </g>
              <g>
                <rect x="8" y="54" fill="#506C7F" width="6" height="4" />
                <rect x="50" y="54" fill="#506C7F" width="6" height="4" />
              </g>
              <g>
                <circle fill="#F9EBB2" cx="11" cy="43" r="2" />
                <circle fill="#F9EBB2" cx="53" cy="43" r="2" />
              </g>
              <path
                opacity="0.15"
                fill="#231F20"
                d="M30,32H18c0-3.313,2.687-6,6-6S30,28.687,30,32z"
              />
            </g>
          </svg>
        ) : (
          <svg
            xmlns="http://www.w3.org/2000/svg"
            // xmlns:xlink="http://www.w3.org/1999/xlink"
            width="100px"
            height="100px"
            viewBox="0 0 128 128"
            aria-hidden="true"
            role="img"
            class="iconify iconify--noto"
            preserveAspectRatio="xMidYMid meet"
          >
            <path
              d="M66.67 49.49s5.07-2.25 9.57-3.1c4.5-.84 8.17-.7 8.17-.7s3.94-5.77 4.5-8.87s.63-5.98.7-6.48c.17-1.2 8.17-1.27 9.85 3.52c1.69 4.79-.42 10.56-1.83 12.67s-2.96 3.66-2.96 3.66s6.9 6.48 8.45 13.66s1.13 17.74 1.13 17.74s-.42 8.02-4.36 8.59c-3.94.56-8.73-3.03-9.43-3.87c-.7-.84-2.06-3.21-4.36-2.46c-2.6.84-3.27 3.84-3.8 6.48c-.66 3.24-1.64 11.5-1.64 11.5s-4.69 1.88-7.51 1.45c-2.82-.42-6.34-3.8-6.34-3.8s.28-4.22-1.13-5.07s-2.82 1.55-2.82 1.55l-5.63 6.19l-7.74-6.34s-.56-2.82-1.41-4.22c-.84-1.41-2.53-4.5-3.1-6.62c-1.4-5.21-.98-15.2-.98-15.2l-8.59-41.95l2.11-1.97s1.97-10.14 5.07-14.5s5.63-5.49 7.32-6.34s6.9-1.41 6.9 0s-2.39 3.24-3.8 5.49s-2.96 5.49-3.8 8.31S47.43 25 47.43 25l3.05.7s1.69-4.08 3.24-6.19c1.55-2.11 3.73-5 6.41-7.04c2.27-1.73 5.42-3.52 8.94-3.52c3.52 0 8.17.99 7.74 3.59c-.23 1.42-2.89.77-5.56 2.53c-3.25 2.14-6.76 5.7-9.36 11.68s-3.52 8.59-3.52 8.59s6.83 6.41 7.53 8.59s.77 5.56.77 5.56z"
              fill="#b68859"
            />
            <path
              d="M35.7 70.14s5.73 3.62 8.26 3.75c3.89.21 9.62-2.09 13.21-6.03s6.55-9.68 6.03-16.68c-.45-6.03-2.51-8.24-2.51-8.24s3.59 3.52 8.8 3.1c5.21-.42 9.43-3.59 11.05-6.76c1.28-2.5 2.46-5.56 1.69-6.12c-.77-.56-3.36 0-3.36 0l-9.81 4.43l-16.82 1.13l-17.6 25.13l1.06 6.29z"
              fill="#6d4938"
            />
            <path
              d="M22.18 30.16c-.7.48.35 3.66 2.82 5.56s5.98 2.46 5.98 2.46l5.21-7.11s-12.67-1.83-14.01-.91z"
              fill="#6d4938"
            />
            <path
              d="M66.76 97.97s2.86 2.63 6.87 2.28c4.01-.35 7.63-2.93 7.63-2.93s-.8 8.21-.87 9.76c-.07 1.55-.21 6.1-.21 6.1s-5.07 5.44-7.81 4.95c-2.75-.49-6.57-5.3-6.5-6.22c.07-.92.66-5.04.8-7.16c.14-2.11.09-6.78.09-6.78z"
              fill="#855b52"
            />
            <path
              d="M56.32 96.72c4.29-.3 7.53-1.95 7.53-1.95s.23 6.01.14 7.6c-.16 2.78-.38 6.1-.66 6.95c-.28.84-9.41 2.04-9.41 2.04s-4.43-3.94-4.43-4.15c0-.21.23-3.21.33-5.44c.07-1.62-.42-6.66-.42-6.66s2.84 1.9 6.92 1.61z"
              fill="#855b52"
            />
            <path
              d="M90.04 85.77s3.03.33 5.07.12c2.04-.21 6.26-1.76 7.32-2.46c1.06-.7 1.83-1.83 1.83-1.83s-.42 8.73-.35 10.42c.07 1.69.07 5.7.07 5.7s-3.31 5.35-6.83 5.7c-3.52.35-7.11-6.15-7.11-6.15s.52-3.43.52-5.54c-.01-2.11-.52-5.96-.52-5.96z"
              fill="#855b52"
            />
            <path
              d="M72.25 114.39c3.72.33 8-1.69 8-1.69s.35 6.41-.63 8.26c-.84 1.6-3.43 3.33-9.78 2.3c-4.12-.67-4.62-3.11-4.86-4.65c-.23-1.55.84-6.9.84-6.9s1.72 2.26 6.43 2.68z"
              fill="#46484a"
            />
            <path
              d="M97.08 99.38c2.96-.1 6.9-1.67 6.9-1.67s.21 6.26-2.18 8.45c-2.75 2.51-9.27 1.74-11.1-.31c-1.83-2.04-.66-8.78-.66-8.78s3.02 2.45 7.04 2.31z"
              fill="#46484a"
            />
            <path
              d="M55.97 108.69c3.46.14 7.65-1.1 7.65-1.1s.52 7.56-3 8.87c-3.74 1.4-8.63 1.08-10.56-1.36c-1.39-1.76-.63-8.17-.63-8.17s2.95 1.62 6.54 1.76z"
              fill="#46484a"
            />
            <path
              d="M39.71 32.88c.69.41 2.11-3.87 11.19-3.8c10 .07 15.27 4.93 16.05 5c.77.07 6.12 0 7.81-.14c1.69-.14 4.65-1.48 4.58-.49s-1.2 6.41-6.19 7.81c-5 1.41-13.44-1.62-15.06-1.76c-1.62-.14-3.03 0-2.89 2.46c.14 2.46-18.37 13.59-18.37 13.59s-13.54-6.31-13.75-7.09c-.21-.77 1.31-2.82 2.56-4.74c1.58-2.44 2.39-5.14 3.24-6.55c.84-1.41 2.18-3.24 2.18-3.24s-2.63.21-4.36-.54c-1.19-.51-4.15-2.49-4.29-3.19s3.31.07 6.12-.21c2.82-.28 8.09-3.24 8.09-3.24s1.76 1.41 2.25 2.75s.49 3.17.84 3.38z"
              fill="#855b52"
            />
            <path
              d="M27.53 66.18s-.81 3.1-.33 6.48c.38 2.65 2.37 5.21 2.72 6.12s.99 3.03.92 3.73c-.07.7.89.87 1.1.35c.26-.63-.02-1.51.7-1.34c.8.19 0 1.74 1.03 1.78c1.08.05.35-1.41 1.13-1.41c.82 0 .09 1.67 1.27 1.64c1.08-.02.07-1.48 1.1-1.45c.84.02.14 1.55 1.31 1.38c.66-.09-.11-1.66.75-1.85c.75-.16.35 1.41 1.22 1.08c.56-.21.75-1.67.54-3.36c-.21-1.69-.84-4.08-1.2-6.01c-.35-1.92-.75-3.97-.77-5.8s-.05-3.97-.05-3.97l-11.44 2.63z"
              fill="#46484a"
            />
            <path
              d="M40.44 50.01c.55.05 2.63-2.25 3.57-3.89s3.43-5.26 7.09-5.68c3.76-.43 5.96 1.55 6.85 4.65c.82 2.86.05 6.15-3.71 7.93c-3.62 1.72-5.02 1.03-8.07 1.92c-1.62.47-4.13 1.88-4.27 2.06c-.14.19.83 6.93-4.5 9.85c-4.46 2.44-11.31 1.5-14.83-2.91c-3.86-4.84-3.14-12.11-.28-15.63c2.83-3.48 6.24-4.75 11.26-3.28c5.62 1.65 6.37 4.93 6.89 4.98z"
              fill="#fff3dd"
            />
            <path
              d="M55.22 47.57c0 2.06-1.31 3.94-3.89 3.99c-2.21.04-3.07-1.79-3.14-3.85c-.09-2.63 1.79-3.95 3.66-3.99c2.43-.05 3.37 1.79 3.37 3.85z"
              fill="#2e2e2e"
            />
            <path
              d="M25.42 52.77s3.66-2.01 4.12-2.25s1.65-.67 2.29.32c.63.99-.39 1.87-1.2 2.22c-.81.35-4.22 2.43-4.22 2.43s-.39 2.57-.04 2.85c.94.75 2.36 1.16 3.52 1.06c1.16-.11 2.08-.6 2.43-.67c.35-.07 1.34-.11 1.55.6c.21.7-.29 1.52-1.16 1.94c-1.04.49-2.08 1.01-3.84.7c-1.37-.24-2.46-.56-3.1-.99c-.63-.42-1.3-1.2-1.3-1.2s-.95 1.16-1.83 1.13c-.88-.04-2.01-.6-2.04-.7c-.04-.11-1.2-1.02-.67-2.01c.53-.99 1.72.35 2.46.46c.74.11 1.2-.39 1.27-.63c.07-.25.04-2.99.04-2.99s-1.58-2.04-1.94-2.39c-.35-.35-1.27-1.51-.32-2.22s1.58 0 1.83.21s2.15 2.13 2.15 2.13z"
              fill="#2e2e2e"
            />
          </svg>
        ))}
    </div>
  );
};

export default Door;
